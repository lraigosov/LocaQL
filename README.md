...
- **Not yet validated for a long-lived, high-QPS deployment, even on Linux/WSL.** Sustained query load can eventually crash the process from a WASM-bridge memory-corruption bug inside the embedded engine itself — a real, third-party issue distinct from the item above (see [Known Divergences](KNOWN-DIVERGENCES.md) Blocking #3 and [`docs/benchmarks.md`](docs/benchmarks.md)). Normal local development use is unaffected; this specifically concerns long-running processes under heavy sustained load.
