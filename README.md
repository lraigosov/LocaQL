...
Job resources expose `statistics.query.statementType` and `numDmlAffectedRows`; simple INSERT/UPDATE/DELETE also expose their unambiguous `dmlStats` category. Query responses expose `numDmlAffectedRows`. This is verified in CI against three official clients exercising the same mutations plus a streaming insert against the resulting table: Python (`google-cloud-bigquery 3.42.3`, `test/clients/python/persistent_ddl_dml.py`), Node.js (`@google-cloud/bigquery 8.3.1`, `test/clients/node/persistent_ddl_dml.js`), and Go (`cloud.google.com/go/bigquery`, `test/clients/go/persistent_ddl_dml.go`). This multi-client coverage caught three real gaps invisible to Python alone: `jobs.insert` previously ignored a client-supplied `jobReference.jobId` and always assigned its own, breaking any client that submits its own job ID and polls using that exact value afterward (the Node.js client's `createQueryJob` does exactly this, per real BigQuery's documented contract) — now honored exactly, validated against BigQuery's own character rules and rejected with `409 Already Exists` if already in use for that project; job statistics (`totalSlotMs`/`processedBytes`/`outputRows`) were rendered as bare JSON numbers instead of BigQuery's actual int64-as-string convention, which the official Go client's generated structs reject outright — now rendered as strings; and result schema field types used GoogleSQL names (`INT64`/`BOOL`/`STRUCT`) instead of BigQuery's REST wire names (`INTEGER`/`BOOLEAN`/`RECORD`), which the Go client's row-value decoder didn't recognize even though its schema parser accepted the alias — now normalized at the REST-response boundary only, leaving the GoogleSQL names this project's own engine needs internally untouched.
