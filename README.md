...
Job resources expose `statistics.query.statementType` and `numDmlAffectedRows`; simple INSERT/UPDATE/DELETE also expose their unambiguous `dmlStats` category. Query responses expose `numDmlAffectedRows`. This is verified in CI with pinned `google-cloud-bigquery 3.42.3` (Python), including SDK-visible affected-row counts for INSERT/UPDATE/MERGE/DELETE and persisted CTAS/DROP behavior, and with the pinned official `@google-cloud/bigquery 8.3.1` Node.js client (`test/clients/node/persistent_ddl_dml.js`) exercising the same mutations plus a streaming insert against the resulting table — multi-client coverage that caught a real gap: `jobs.insert` previously ignored a client-supplied `jobReference.jobId` and always assigned its own, which silently broke any client (the official Node.js one among them) that submits its own job ID and polls using that exact value afterward, as real BigQuery's documented contract allows. A client-supplied `jobReference.jobId` is now honored exactly (validated against BigQuery's own character rules, rejected with `409 Already Exists` if already in use for that project).
