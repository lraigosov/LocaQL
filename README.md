...
- **Run LocaQL itself on Linux (including WSL on Windows) — not verified to work natively on Windows or macOS.** The query engine's WASM-based analyzer traps at runtime outside Linux (discovered in Sesión 85; see [Known Divergences](KNOWN-DIVERGENCES.md) Blocking #2): the binary builds fine for every platform, but every query fails. Building/cross-compiling from any OS is unaffected — only running it natively on Windows/macOS is currently broken. **If you just want to run LocaQL on Windows/macOS rather than develop it, the [published Docker image](#published-image-github-container-registry) sidesteps this entirely** — it runs Linux internally regardless of host OS, the same way Docker Desktop already does for every other container on those platforms.
- **Sustained, heavy query load can eventually crash the process**, from a WASM-bridge memory-corruption bug inside the embedded engine itself — a real, third-party issue distinct from the item above, reproduced on Linux/WSL too (see [Known Divergences](KNOWN-DIVERGENCES.md) Blocking #3 and [`docs/benchmarks.md`](docs/benchmarks.md)). Normal local development use is unaffected. Running via [`cmd/locaql-supervisor`](cmd/locaql-supervisor) (the Docker image's entrypoint) absorbs this automatically — it restarts the emulator process on an unexpected exit — so the service stays available; running the bare `locaql` binary directly gets the same protection by adding `--self-restart` (`locaql start --self-restart`), which re-execs itself as a supervised child instead of running the server in that process directly. Without `--self-restart` (the default), a direct `locaql start` still has no such protection.
...
- Completed query jobs report logical `totalBytesProcessed` from the catalog/session source rows actually materialized, rather than from the result rows. A read-only query with one physical partitioned-table occurrence and an AST-proven `partition_column = typed_literal` predicate materializes only the matching time/ingestion partition or complete integer-range bucket. `totalSlotMs` and dry-run estimates remain synthetic, and there is no physical column-cost model yet.
...
Job resources expose `statistics.query.statementType` and `numDmlAffectedRows`; simple INSERT/UPDATE/DELETE also expose their unambiguous `dmlStats` category. Query responses expose `numDmlAffectedRows`. This is verified in CI against seven official clients exercising the same mutations plus a streaming insert against the resulting table: Python (`google-cloud-bigquery 3.42.3`, `test/clients/python/persistent_ddl_dml.py`), Node.js (`@google-cloud/bigquery 8.3.1`, `test/clients/node/persistent_ddl_dml.js`), Go (`cloud.google.com/go/bigquery`, `test/clients/go/persistent_ddl_dml.go`), Java (`com.google.cloud:google-cloud-bigquery`, `test/clients/java/`), Ruby (`google-cloud-bigquery 1.64.0`, `test/clients/ruby/persistent_ddl_dml.rb`), PHP (`google/cloud-bigquery`, `test/clients/php/persistent_ddl_dml.php`) and the official `bq` CLI (`test/clients/bq/persistent_ddl_dml.sh` — see [bq CLI](#bq-cli-a-hand-written-discovery-document) below for why it needs its own discovery document). This multi-client coverage caught six real gaps invisible to Python alone, each fixed at the root rather than worked around in the test — the PHP client passed cleanly against every one of them on its first real run, a working confirmation that those earlier fixes generalized rather than being coincidentally client-specific:

- `jobs.insert` ignored a client-supplied `jobReference.jobId` and always assigned its own, breaking any client that submits its own job ID and polls using that exact value afterward (the Node.js client's `createQueryJob` does exactly this, per real BigQuery's documented contract) — now honored exactly, validated against BigQuery's own character rules and rejected with `409 Already Exists` if already in use for that project.
- Job statistics (`totalSlotMs`/`processedBytes`/`outputRows`) were rendered as bare JSON numbers instead of BigQuery's actual int64-as-string convention, which the official Go client's generated structs reject outright — now rendered as strings.
- Result schema field types used GoogleSQL names (`INT64`/`BOOL`/`STRUCT`) instead of BigQuery's REST wire names (`INTEGER`/`BOOLEAN`/`RECORD`), which the Go client's row-value decoder didn't recognize even though its schema parser accepted the alias — now normalized at the REST-response boundary only, leaving the GoogleSQL names this project's own engine needs internally untouched.
- A job resource never echoed back the submitted query text in `configuration.query.query`, crashing the official Java client outright when it tried to reconstruct a `Job` object from the response — now included alongside `priority`.
...
- Every append/load/Storage Write operation assigns ingestion-time partition IDs. `_PARTITIONTIME` (`TIMESTAMP`) and `_PARTITIONDATE` (`DATE`) are executable, read-only pseudocolumns for ingestion-time tables and are omitted from `SELECT *`, matching BigQuery's wildcard behavior. Persistent `UPDATE`/`DELETE` preserve exact IDs for surviving rows—even duplicate user rows in different partitions—and `INSERT`, including an omitted target-column list, assigns the current partition. Time partition expiration is enforced lazily on catalog access and removes only expired dated partitions; `NULL` partition values remain in `__NULL__`.
...
The pseudocolumn implementation uses engine-only columns during isolated query execution and removes them before REST results or catalog commits; public schemas never contain the internal names. The REST TIMESTAMP wire value is emitted as Unix epoch microseconds, so official clients receive timezone-aware datetime objects. Table discovery and `requirePartitionFilter` eligibility now inspect the GoogleSQL AST, so column names inside literals/comments no longer count as filters. Logical pruning is intentionally conservative: equality against a typed literal, optionally inside `AND`, one physical table occurrence, SELECT only. `OR`, parameters, casts/functions, inequality/range predicates, repeated table occurrences and anything ambiguous materialize all rows; DDL/DML always materializes the complete target to preserve untouched partitions. This reduces source-row bytes for the proven subset but is not BigQuery billing parity, physical column pruning, clustering optimization or a full cost model. Direct-pseudocolumn projection detection remains lexical. Partitioned-table creation currently uses the REST API/official clients; GoogleSQL `CREATE TABLE ... PARTITION BY ... CLUSTER BY ...` is not part of the persistent DDL subset.
...
Run the datasets/tables/streaming-insert/persistent-DDL-DML lifecycle suite:

```bash
wsl -d Ubuntu-24.04 -- bash -lc 'cd /mnt/f/GitHub/LocaQL && go run ./cmd/locaql conformance --base-url http://localhost:9050 --cases test/conformance/cases/lifecycle.yaml --report-json test/conformance/reports/lifecycle-report.json --report-md test/conformance/reports/lifecycle-report.md'
```

`foundation.yaml`/`pagination.yaml` only check an HTTP status code against a fixed path. `lifecycle.yaml` uses a richer declarative shape (`internal/conformance/runner.go`) built for this suite: each case may carry a JSON request `body`, `expected_body_contains` substring checks against the response body, and `setup`/`cleanup` request lists — `cleanup` always runs, even when `setup` or the case itself fails, so a case builds (and always tears down) the exact dataset/table/rows it needs and the suite stays runnable against a fresh, empty server rather than depending on fixtures created by an earlier run. This exercises the datasets/tables/streaming-insert/`queries` REST surface directly, independent of any one official client library.

Per-case lifecycle (`Runner.Run` in `internal/conformance/runner.go`):

```mermaid
flowchart LR
	Setup["Run setup steps"] -->|"any step fails"| SkipMain["Skip the case's own request"]
	Setup -->|"all steps succeed"| Main["Run the case's own request"]
	Main --> Assert{"status matches and every\nexpected_body_contains found?"}
	Assert -->|yes| Pass[Case passed]
	Assert -->|no| Fail[Case failed]
	SkipMain --> Cleanup["Run cleanup steps\n(always, regardless of outcome)"]
	Pass --> Cleanup
	Fail --> Cleanup
	Cleanup --> Result[CaseResult appended to Report]
```
...
**Baseline (Sesión 59, `go test -list '.*'` counts, not hand-counted):** 218 unit tests across `internal/server` (193), `cmd/locaql` (9), `cmd/locaql-ui` (2), `internal/capabilities` (1), `internal/workspace` (9) and `internal/conformance` (4), plus 15 more under the `e2e` build tag (see [End-to-End Console Tests](#end-to-end-console-tests)) and 14 declarative conformance cases (`test/conformance/cases/foundation.yaml` + `pagination.yaml` + `lifecycle.yaml`, see [Conformance Baseline](#conformance-baseline)). This is a point-in-time snapshot for onboarding context, not a target to keep updated by hand — `.github/workflows/ci.yml`'s `test`/`e2e` jobs are the actual source of truth for whether the suite is green today.
...
[`CHANGELOG.md`](CHANGELOG.md) is the curated, user-facing summary of what changed release to release, in [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) format — distinct from `devlog.md` (gitignored, not published), which is this project's full internal session-by-session build log. Releases use the same linear promotion as the code: when a same-repository `feature/* -> dev` PR opens or updates, `.github/workflows/prepare-release.yml` detects the SemVer bump from `[Unreleased]`, dates it and commits the release bookkeeping back to that work branch without bypassing protected `dev`; merging the resulting `dev -> main` PR makes `.github/workflows/auto-tag-release.yml` tag that exact merge commit and invoke `.github/workflows/release.yml`. No `release/*` branch or release-only PR is created, while automatic validation, the multi-arch GHCR image, cross-platform binaries, SBOM and GitHub Release remain intact. See `CHANGELOG.md`'s "How releases are cut" section for the guards and exact order. Pushing a `vX.Y.Z` tag by hand still works as a recovery path.
...
| `python-client` | ubuntu-latest | Pinned official `google-cloud-bigquery`/`google-cloud-storage` clients exercise uploads, persistent SQL, partitioning, and Cloud Storage downloads (`test/clients/python/`) |
| `node-client` | ubuntu-latest | Pinned official `@google-cloud/bigquery` Node.js client exercises persistent SQL mutations and streaming inserts (`test/clients/node/`) |
| `go-client` | ubuntu-latest | Official `cloud.google.com/go/bigquery` client exercises the same mutations and streaming inserts (`test/clients/go/`) |
...
| `php-client` | ubuntu-latest | Official `google/cloud-bigquery` PHP client exercises the same mutations and streaming inserts (`test/clients/php/`) |
