...
[`CHANGELOG.md`](CHANGELOG.md) is the curated, user-facing summary of what changed release to release, in [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) format — distinct from `devlog.md` (gitignored, not published), which is this project's full internal session-by-session build log. See `CHANGELOG.md`'s own "How releases are cut" section for the exact, fully automated process: every push to `main` runs `.github/workflows/prepare-release.yml`, which detects whether `[Unreleased]` has content, works out the SemVer bump from which `### ` subsections are present, dates the section, and opens a `release/vX.Y.Z` → `main` PR — no manual version number, no manual trigger. Merging that PR (the one step branch protection still requires a human for) triggers `.github/workflows/auto-tag-release.yml`, which tags the merge commit and calls `.github/workflows/release.yml` directly — validating, publishing the multi-arch GHCR image, cross-compiling every platform binary, and creating the GitHub Release with those binaries and the SBOM attached. Pushing a `vX.Y.Z` tag by hand still works too (`release.yml` keeps its original tag-push trigger) for anyone who wants to skip the PR step.
