...
LocaQL is licensed under a proprietary Community/Evaluation [End User License Agreement (EULA)](LICENSE). The Software is free to use strictly for academic research, personal education, training, and limited personal local development testing under that license; production, hosting, OEM, or resale use requires a separate [Commercial License](COMMERCIAL_LICENSE.md).

Read the [`LICENSE`](LICENSE) file for the full terms, conditions, and strict operational boundaries of "Limited Local Development Testing", including the distinction between Licensor's own IP and open-source Third-Party Components. Read the [`NOTICE`](NOTICE) file and [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md) before using, deploying, or building upon this project — they explain the attribution you're required to carry forward, and clarify that LocaQL is not affiliated with Google or BigQuery.

By submitting a contribution (including code, documentation, bug reports, or suggestions), you agree to assign and transfer all intellectual property and copyright ownership of that contribution to Luis Raigoso, as detailed in [Section 7 (Contributions and Intellectual Property Assignment)](LICENSE) of the EULA, or grant an unrestricted, perpetual, and irrevocable license to the Licensor to use and re-license your contributions under any model. **Future pull requests additionally require explicit acceptance of the [Contributor License Agreement](CLA.md)** before they can be merged — see that document for the mechanism.
