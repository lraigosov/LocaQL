...
Read the [`LICENSE`](LICENSE) file for the full terms. Read the [`NOTICE`](NOTICE) file and [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md) before using, deploying, or building upon this project — they explain the attribution you're required to carry forward, and clarify that LocaQL is not affiliated with Google or BigQuery.

By submitting a contribution (including code, documentation, bug reports, or suggestions), you agree to assign and transfer all intellectual property and copyright ownership of that contribution to Luis Raigoso, or grant an unrestricted, perpetual, and irrevocable license to the Licensor to use and re-license your contributions under any model — this is independent of, and does not change, the Apache-2.0 license governing the Software itself. **Pull requests require explicit acceptance of the [Contributor License Agreement](CLA.md)** before they can be merged — see that document for the mechanism.
