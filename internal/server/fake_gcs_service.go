...
// Google Cloud Storage JSON API: buckets.insert/list, objects.insert (media
// and multipart upload variants), objects.get, objects.list,
...
// inventing partial ones: resumable uploads, IAM/ACLs, object
// versioning/generations beyond a
...
func renderGCSObjectResource(r *http.Request, bucket, object string, info os.FileInfo, md5sum string) map[string]any {
	modTime := info.ModTime().UTC().Format(time.RFC3339)
	bucketPath := url.PathEscape(bucket)
	objectPath := url.PathEscape(object)
	selfLink := fmt.Sprintf("/storage/v1/b/%s/o/%s", bucketPath, objectPath)
	mediaLink := fmt.Sprintf("/download/storage/v1/b/%s/o/%s?alt=media", bucketPath, objectPath)
	if r != nil && r.Host != "" {
		scheme := "http"
		if r.TLS != nil {
			scheme = "https"
		...
		mediaLink = scheme + "://" + r.Host + mediaLink
	}
	return map[string]any{
		"kind":        "storage#object",
...
func (s *Server) gcsBucketScope(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet, http.MethodHead:
		if r.URL.Query().Get("alt") == "media" {
			s.gcsDownloadObject(w, r, root, bucket, object)
			return
		}
		...
		s.gcsGetObjectMetadata(w, r, root, bucket, object)
...
func (s *Server) gcsListObjects(w http.ResponseWriter, r *http.Request, root, bucket string) {
	...
	for _, name := range names {
		...
		items = append(items, renderGCSObjectResource(r, bucket, name, info, md5sum))
	}
	writeJSON(w, http.StatusOK, map[string]any{"kind": "storage#objects", "items": items})
}

func (s *Server) gcsGetObjectMetadata(w http.ResponseWriter, r *http.Request, root, bucket, object string) {
	...
	writeJSON(w, http.StatusOK, renderGCSObjectResource(r, bucket, object, info, md5sum))
}

// gcsDownloadScope implements the media URL generated by official Cloud
// Storage clients: /download/storage/v1/b/{bucket}/o/{url-escaped-object}.
// Keep the older objects.get?alt=media path in gcsBucketScope as a compatible
// alias, since both forms are valid ways for callers to request object bytes.
func (s *Server) gcsDownloadScope(w http.ResponseWriter, r *http.Request) {
	root, err := fakeGCSRoot()
	if err != nil {
		writeGCSError(w, http.StatusServiceUnavailable, err.Error())
		return
	...
	bucket, tail, hasTail := strings.Cut(rest, "/")
	object := strings.TrimPrefix(tail, "o/")
	if !hasTail || strings.TrimSpace(bucket) == "" || object == tail || object == "" {
		writeGCSError(w, http.StatusNotFound, "unrecognized storage download path")
		return
	}
	s.gcsDownloadObject(w, r, root, bucket, object)
}

func (s *Server) gcsDownloadObject(w http.ResponseWriter, r *http.Request, root, bucket, object string) {
	path, err := fakeGCSObjectPath(root, bucket, object)
	if err != nil {
		writeGCSError(w, http.StatusBadRequest, err.Error())
		return
	...
	w.Header().Set("ETag", fmt.Sprintf("%q", md5sum))
	w.Header().Set("X-Goog-Hash", "md5="+md5sum)
	http.ServeContent(w, r, object, info.ModTime(), f)
}
